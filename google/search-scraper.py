...
def writeToDisk(ads, query, initialize=False):
    ...
    with open(outpath, mode) as f:
        ...
        for adText, isPolitical in ads:
            ...
            writer.writerow([adText, category, 'Google', query])
...
def scrape_search_ads(queries, newFile):
    ...
    writeToDisk([], None, newFile) # initialize output file
...
def remove_duplicates():
    ...
    before = len(df.index)
    df.drop_duplicates(subset=['AdText'], keep='last', inplace=True)
